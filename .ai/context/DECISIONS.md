...
## Decisión pendiente de implementar: eliminar el rol `trabajador` (2026-09-11)

**Contexto**: el sistema de roles `admin`/`trabajador` (migraciones `022`/`023`, ver arriba) fue pensado para que el personal de campo tuviera acceso solo a la app mobile, sin panel web. **Decisión de Mateo**: sacar el rol `trabajador` — el proyecto queda con un solo rol (`admin`). **Estado: NO IMPLEMENTADO todavía** — el código actual (`middleware.ts`, `app/actions/users.ts`, RLS vía `is_admin()`) sigue teniendo el sistema de dos roles tal como está descrito en `.ai/context/ARCHITECTURE.md`. Antes de implementar esto hay que resolver qué pasa con la app mobile, que hoy depende de que exista una cuenta `trabajador` para autenticarse contra `app/api/**` — sacar el rol sin un reemplazo probablemente rompe el acceso del personal de campo desde el celular. Ver `.ai/context/CURRENT_STATE.md`.
